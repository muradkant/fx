...
fn runProviderLogin(alloc: Allocator, cfg: Config, provider: model_provider.ProviderId) !void {
    switch (provider) {
        ...
        .opencode => {
            const key = io_mod.getenv("OPENCODE_API_KEY") orelse return error.MissingOpenCodeApiKey;
            var session = opencode_session.Session{ .api_key = try alloc.dupe(u8, key) };
            defer session.deinit(alloc);
            try opencode_session.saveNewSession(alloc, session);
        },
        .cline => {
            if (io_mod.getenv("CLINE_API_KEY")) |key| {
                var session = cline_session.Session{ .api_key = try alloc.dupe(u8, key) };
                defer session.deinit(alloc);
                try cline_session.saveNewSession(alloc, session);
            } else {
                try cline_oauth.runLogin(alloc, cfg.gateway_provider.oauth_transport, cfg.url_opener);
            }
        },
    }
}
...
fn writeProviderLoginFailure(alloc: Allocator, deps: RunDeps, provider: model_provider.ProviderId, caller: ProviderActivationCaller, err: anyerror) !void {
    ...
    try writeProviderActivationError(alloc, deps, caller, switch (err) {
        ...
        error.MissingOpenCodeApiKey => "set OPENCODE_API_KEY to your opencode.ai key first",
    ...
    switch (parsed_command) {
        ...
        .login => |rest| {
            ...
            try writeStdout(deps, switch (login_provider) {
                ...
                .opencode => "Signed in with OpenCode.\n",
