...
test "requested credential source follows provider authority" {
    const cases = [_]struct {
        ...
        .{ .provider = .opencode, .preferred = .fx_login, .required = .opencode_api_key },
        .{ .provider = .cline, .preferred = .fx_login, .required = .cline_account },
    };
    for (cases) |case| {
...
pub const StatusSnapshot = struct {
    ...
    pub fn missingHelp(self: StatusSnapshot, surface: MissingHelpSurface) ?[]const u8 {
        ...
        return switch (required_source) {
            ...
            .cline_account, .cline_api_key => switch (surface) {
                .cli => credentials.missing_cline_credential_message,
                .interactive => credentials.missing_cline_interactive_credential_message,
            },
...
test "provider selection preserves an already-authorized routed credential" {
    ...
    for (cases) |case| {
        ...
        try std.testing.expectEqual(ProviderCredentialSelection.unchanged, try runtime.selectForProvider(alloc, case.provider, null));
