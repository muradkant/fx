    ...
    return struct {
            ...
            switch (selection) {
                ...
                .missing => try app.writeDomainNotice(.{
                    ...
                    .body = switch (provider) {
                        ...
                        .opencode => credentials.missing_opencode_interactive_credential_message,
                        .cline => credentials.missing_cline_interactive_credential_message,
                    },
                }, true),
                .failed => |failure| {
            ...
            if (provider != .gateway) {
                try app.writeDomainNotice(.{
                    ...
                    .body = if (provider == .grok)
                        ...
                        credentials.missing_cline_interactive_credential_message
                    else
            ...
            if (task.credential == null and !hostManagesAuth(app)) {
                if (request.allow_login) {
                    switch (target) {
                        ...
                        .opencode => try beginOpenCodeSignIn(app),
                        .cline => try beginClineSignInForProviderSwitch(app),
                        .gateway => {},
                    }
                }
        ...
        fn beginCredentialRepair(
            ...
            switch (failure.source) {
                ...
                .opencode_api_key => try beginOpenCodeSignIn(app),
                .cline_account, .cline_api_key => try beginClineSignInForProviderSwitch(app),
                .vercel_oidc_token,
                .ai_gateway_api_key,
                .stored_key,
                .host_managed,
...
const TestAuth = struct {
    ...
    fn openClineSignInPickerFromRoot(self: *TestAuth, alloc: std.mem.Allocator) !bool {
        return self.openSignInPicker(alloc);
    }

    fn openChatGptSignInPickerForProviderSwitch(self: *TestAuth, alloc: std.mem.Allocator) !bool {
        return self.openSignInPicker(alloc);
    }

    fn openGrokSignInPickerForProviderSwitch(self: *TestAuth, alloc: std.mem.Allocator) !bool {
        return self.openSignInPicker(alloc);
    }

    fn openClineSignInPickerForProviderSwitch(self: *TestAuth, alloc: std.mem.Allocator) !bool {
        return self.openSignInPicker(alloc);
    }
